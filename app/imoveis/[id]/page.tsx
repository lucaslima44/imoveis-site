...
function PropertyDetail({ property }: { property: Property }) {
  ...
  const specs = [
    ...
    { icon: Car, label: "Vagas de Garagem", value: String(property.parkingSpots) },
  ...
  return (
    <div className="bg-cream-100 min-h-screen pt-24">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-6">
        <Link
          href="/imoveis"
          className="inline-flex items-center gap-2 text-navy-500 text-sm font-body hover:text-gold-500 transition-colors duration-200 group"
        >
          <ArrowLeft
            size={15}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
              {specs.map((spec, i) => (
                <div
                  key={i}
                  className="bg-cream-50 p-5 flex flex-col items-center text-center gap-2 shadow-[0_2px_20px_rgba(15,30,43,0.08)] hover:shadow-[0_8px_40px_rgba(15,30,43,0.14)] transition-shadow duration-300"
                >
                  <spec.icon size={20} className="text-gold-500" />
                  <span className="font-display text-xl font-semibold text-navy-900">
                    {spec.value}
                  </span>
                  <span className="font-body text-navy-500 text-xs">
                    {spec.label}
                  </span>
                </div>
              ))}
            ...
            <div>
              ...
              <p className="font-body text-navy-600 leading-relaxed text-[15px]">
          ...
          <div className="lg:col-span-1">
            <div className="bg-cream-50 shadow-[0_2px_20px_rgba(15,30,43,0.08)] p-7 sticky top-32">
              <p className="font-body text-navy-500 text-xs tracking-[0.15em] uppercase mb-1">
                Valor
              </p>
              <p className="font-display text-3xl font-semibold text-navy-900 mb-6">
              ...
              <ul className="space-y-0 mb-8">
                {infoRows.map(([label, val]) => (
                  <li
                    key={label}
                    className="flex justify-between items-center text-sm font-body border-b border-cream-200 py-3 last:border-0"
                  >
...
export default function PropertyPage({ params }: Props) {
  ...
  // ↑ notFound() throws — but TypeScript may not narrow here in strict mode.
  // Delegating to a child component removes the ambiguity entirely.
